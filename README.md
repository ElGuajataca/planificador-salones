...
- **Personas por mesa editable mesa a mesa** — al seleccionar una mesa, la barra flotante tiene un selector de 1 a 12 pax (impares incluidos); las sillas dibujadas, la etiqueta y el conteo de aforo se ajustan al instante.
- **Silla individual imantada** — pieza suelta de la paleta; al soltarla cerca de una mesa se pega a su borde y se mueve y gira con ella (arrástrala lejos para despegarla). Suma 1 al aforo sin contar como mesa.
...
- **Inventario editable desde la interfaz** — añadir o editar sillas (✎ junto al selector), crear piezas nuevas (＋ nueva) o editar las existentes (✎ en cada tarjeta: medidas, asientos, stock, colores). Formas disponibles: rectangular, redonda, serpentina/media luna (U) y **personalizada** — se dibuja la silueta esquina por esquina en un lienzo con cuadrícula y se escala al ancho × fondo indicados en cm. Las piezas base se pueden restaurar a su valor original.
